...
import a17_재네릭.User.CompanyUser;
import a17_재네릭.User.PersonalUser;

public class ControllerTest {
	
	public CMRespDto<? extends User> getProfile(String username) {
		PersonalUser personalUser =  new PersonalUser("junul", "1234", "김준일" , "j@gmail.com", "a20220001");
		...
		if(username.equals(personalUser.getUsername())){
			return new CMRespDto<PersonalUser>(1, "개인 회원 프로필", personalUser);
		}else if(username.equals(companyUser.getUsername())) {
			return new CMRespDto<CompanyUser>(1, "기업 회원 프로필", companyUser);
		}else {
