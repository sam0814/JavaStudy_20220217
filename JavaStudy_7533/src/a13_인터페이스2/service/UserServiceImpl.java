...
public class UserServiceImpl implements UserService{
	...
	public UserServiceImpl() {
		...
		//회원을 저장 할 배열의 크기를 지정하고 생성 된 배열을 UserData 객체의 users 배열 변수에 set 하는 것
		
		scanner.nextLine();
		//위의 nextInt()후에 버퍼에 저장된 엔터를 날려주기 위함.
		this.userData = UserData.getInstance(new User[scanner.nextInt()]);
	}

	@Override
	public void addUser(User user) {
		User[] users = userData.getUsers(); //user배열 주소 가져옴.
		for(int i = 0; i < users.length; i++) { //user배열에서 빈 공간이 있으면 입력한 user 객체를 배열에 저장하겠다.
			if(users[i] == null) {
				users[i] = user;
				break;
			}
